...
const findDraftSlot = (draftOrder, rosterID) =>
	draftOrder.findIndex((draftRosterID) => String(draftRosterID) === String(rosterID));
...
const buildFromScratch = (rosters, previousOrder, rounds, picks, regularSeasonLength) => {
	...
	for(const pick of picks) {
		if(String(pick.owner_id) === String(pick.roster_id) || pick.round > rounds) continue;
		const draftSlot = findDraftSlot(draftOrder, pick.roster_id);
		if(draftSlot < 0) {
			console.error(`Invalid roster ID in traded pick: ${pick.roster_id}`);
			continue;
		}
		draft[pick.round - 1][draftSlot] = pick.owner_id;
	}
...
const completedNonAuction = ({players, draft, picks, draftOrder, rounds}) => {
	...
	for(const pick of picks) {
		if(String(pick.owner_id) === String(pick.roster_id) || pick.round > rounds) continue;
		const draftSlot = findDraftSlot(draftOrder, pick.roster_id);
		if(draftSlot < 0 || !draft[pick.round - 1][draftSlot]) {
			console.error(`Invalid roster ID in completed traded pick: ${pick.roster_id}`);
			continue;
		}
		draft[pick.round - 1][draftSlot].newOwner = pick.owner_id;
	}
	return draft;
}
...
const completedAuction = ({players, draft, draftOrder, draftOrderObj}) => {
	...
	for (const roster in rosters) {
		const col = findDraftSlot(draftOrder, roster);
		if(col < 0) {
			console.error(`Invalid roster ID in auction draft: ${roster}`);
			continue;
		}
