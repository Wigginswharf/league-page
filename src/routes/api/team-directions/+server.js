...
const fetchJSON = async (url) => {
  ...
  if (!response.ok)
    throw new Error(`Ratings source returned ${response.status}`);
...
const optimalLineupValue = (playerIDs, values, rosterPositions) => {
  const players = playerIDs.map((id) => values.get(String(id))).filter(Boolean);
  const used = new Set();
  let value = 0;
...
const calculatePickValues = (rosters, tradedPicks, season, rounds = 3) => {
  ...
  const totals = Object.fromEntries(
    rosters.map((roster) => [roster.roster_id, 0]),
  );
...
const getDefendingChampionOwner = async (previousLeagueID) => {
  ...
  try {
    ...
    const championship = bracket.find(
      (matchup) => matchup.p === 1 && matchup.w,
    );
    return (
      rosters.find((roster) => roster.roster_id === championship?.w)
        ?.owner_id || null
    );
  } catch (error) {
...
export async function GET() {
  ...
  const defendingChampionOwner = await getDefendingChampionOwner(
  ...
  const teams = rosters.map((roster) => {
    const playerValues = (roster.players || [])
      ...
      .filter(Boolean);
    const dynastyPlayerValue = playerValues.reduce(
      (sum, player) => sum + player.dynastyValue,
      0,
    );
    ...
    return {
      ...
      currentStrength: optimalLineupValue(
        roster.players || [],
        values,
  ...
  for (const team of teams) {
    team.windowScore =
      (13 - team.currentStrengthRank) * 0.65 +
      (13 - team.futureStrengthRank) * 0.35;
  }
  ...
  for (const team of teams) {
    ...
    if (
      (team.defendingChampion &&
        team.currentStrengthRank <= 8 &&
        team.futureStrengthRank <= 5) ||
